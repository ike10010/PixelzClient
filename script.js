...
// Download handling - direct link now points to 1.1.5 jar, just toast
async function handleDownload(e){
  ...
  const jarUrls = [
    '../build/libs/pixelz-client-1.1.5.jar',
    './pixelz-client-1.1.5.jar',
    '/build/libs/pixelz-client-1.1.5.jar'
  ];
  for(const url of jarUrls){
    try{
      ...
      if(res.ok){
        ...
        toast('Downloading pixelz-client-1.1.5.jar');
        return;
      }
    }catch{}
  }
  ...
  const blob = new Blob([
`Pixelz Client 1.1.5 - 1.21.11 Fabric
Build: ./gradlew build
Jar: build/libs/pixelz-client-1.1.5.jar
